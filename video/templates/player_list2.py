...
from moviepy.video.fx import Blink as vfx_blink
...
class PlayerList2(VideoTemplate):
    ...
    def process(self, user, video_id, parameters):
        ...
        try:
            ...
            # 开场：将所有原图统一缩放，快速自右向左播放一遍
            selected_images = body_paths if len(body_paths) > 0 else []
            # 开场只轮播一轮，每张图片使用 blink(0.5, 0.5)，每段时长 1s
            blink_on = 0.5
            blink_off = 0.5
            intro_seg = blink_on + blink_off
            time_used_intro = len(selected_images) * intro_seg + 0.3
            if time_used_intro > start:
                selected_images = selected_images[:int((start - 0.3) // intro_seg)]
            start_time = 0.3
            if selected_images:
                for idx, img_path in enumerate(selected_images):
                    try:
                        base_img = self.img_utils.trim_image(img_path).convert("RGBA")
                    except Exception:
                    ...
                    base_img = base_img.resize((target_w, target_h), PilImage.LANCZOS)

                    tmp_intro_path = os.path.join(self.tmps, f"intro_trim_{idx}.png")
                    try:
                        base_img.save(tmp_intro_path, format="PNG", optimize=True)
                    except Exception:
                        base_img.save(tmp_intro_path)

                    # 每个开场片段 1s：0.5s 显示 + 0.5s 关闭（blink），无位移/缩放
                    clip = ImageClip(tmp_intro_path).with_start(start_time).with_duration(intro_seg).with_position("center")
                    clip = clip.with_effects([vfx_blink(blink_on, blink_off)])
                    clips.append(clip)
                    start_time += intro_seg
            ...
            title_clip = title_clip.with_position(("center", target_y))
            clips.append(title_clip)

            # 若 start 时间较长：开场只播放一轮后，第一张内容图提前进入；其后内容保持原计划开始时间
            intro_first_t = start_time if start > start_time else start
            accum = 0.0
            for i in range(len(body_paths)):
                duration_stay = content_durations[i] if i != 0 else content_durations[0] + start - start_time
                baseline_t = start_time + accum
